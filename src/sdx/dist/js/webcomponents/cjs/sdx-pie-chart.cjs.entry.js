...
const PieChart = class {
    ...
    dataChanged() {
        this.dataState = webcomponentHelpers.parseJson(this.data) || [];
    }
    componentWillLoad() {
        this.dataState = webcomponentHelpers.parseJson(this.data) || [];
    }
    ...
    render() {
        ...
        const segmentsTimeline = anime_es.anime.timeline();
        const legendItemsTimeline = anime_es.anime.timeline();
        return (index.h("div", { class: this.getComponentClassNames() }, index.h("div", { class: "wrapper" }, index.h("div", { class: "chart-container" }, index.h("div", { class: "chart" }, this.size === "small" &&
            index.h("svg", { viewBox: `0 0 ${this.viewBox} ${this.viewBox}`, style: { transform: `rotate(${this.widthToDeg(this.hundredPercent)}deg)` } }, index.h("circle", { stroke: this.getSegmentColor(), cx: center, cy: center, r: this.radius })), this.dataToSegments().map((segment) => {
            return (index.h("svg", {
            ...
            }, index.h("circle", { ref: (el) => this.animateSegment(Object.assign(Object.assign({}, segment), { circle: el }), segmentsTimeline), style: { display: "none" /* will be displayed during animation */ }, stroke: this.getSegmentColor(segment.d), cx: center, cy: center, r: this.radius })));
        })), index.h("div", { class: "metadata", "aria-hidden": "true" }, index.h("div", { class: "value" }, this.value), this.size !== "small" &&
            index.h("div", { class: "description" }, this.description))), this.hasLabelsOrSrHints() &&
            index.h("ul", { class: { legend: true, "sr-only": this.size === "small" } }, this.dataState.filter(this.hasLabelOrSrHint).map((d) => {
                return (index.h("li", { key: Math.random() /* make sure elements are not reused (causing glitchy animations) */, ref: (el) => this.animateLegendItem(el, legendItemsTimeline), style: { opacity: "0" } }, index.h("span", { "aria-hidden": "true" }, index.h("sdx-icon", { "icon-name": "icon-record-filled", style: { color: this.getSegmentColor(d) } }), " ", d.label), d.srHint && index.h("span", { class: "sr-only" }, d.srHint)));
            })))));
    }
...
exports.sdx_pie_chart = PieChart;
