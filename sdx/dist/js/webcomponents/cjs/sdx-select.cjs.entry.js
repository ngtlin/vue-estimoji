...
const bodyScrollLock = _commonjsHelpers.unwrapExports(bodyScrollLock_min);
...
const selectReducer = (state = {}, action) => {
    switch (action.type) {
        ...
        case "TOGGLE_OPTION_EL":
            return Object.assign(Object.assign({}, state), { optionElsBatch: immutabilityHelpers.toggle(state.optionElsBatch, action.optionEl) });
        case "TOGGLE_OPTGROUP_EL":
            return Object.assign(Object.assign({}, state), { optgroupElsBatch: immutabilityHelpers.toggle(state.optgroupElsBatch, action.optgroupEl) });
        case "COMMIT_OPTION_ELS_BATCH":
            ...
            if (state.optionEls !== state.optionElsBatch) { // options have changed
                optionElsSorted = [...state.optionElsBatch].sort(webcomponentHelpers.sortByAppearanceInDomTree);
            }
...
const Select = class {
    ...
    componentWillLoad() {
        this.updateHiddenFormInputEl();
        this.store = storeHelpers.createAndInstallStore(this, selectReducer, this.getInitialState());
    ...
    optionElMatchesFilter(el, keyword) {
        ...
        if (this.filterFunction) {
            filterFunction = webcomponentHelpers.parseFunction(this.filterFunction);
        }
        ...
        if (this.isAutocomplete()) {
            ...
            if ((breakpointHelpers.isDesktopOrLarger() && visibleOptionElsCount >= Select.maxAutocompleteOptionsDesktop)
                ||
                    (!breakpointHelpers.isDesktopOrLarger() && visibleOptionElsCount >= Select.maxAutocompleteOptionsMobile)) {
                match = false;
            }
        }
    ...
    setFocussedEl(which) {
        ...
        if (which === "previous" || which === "next") {
            ...
            if (which === "previous") {
                let previousElement = webcomponentHelpers.getPreviousFromList(this.optionElsSorted, focussedEl);
                let count = 0;
                while (previousElement !== focussedEl && (previousElement.disabled || previousElement.style.display === "none") && (count < this.optionElsSorted.length)) {
                    previousElement = webcomponentHelpers.getPreviousFromList(this.optionElsSorted, previousElement);
                    count++;
                }
                this.focussedEl = previousElement;
            }
            else { // "next"
                let nextElement = webcomponentHelpers.getNextFromList(this.optionElsSorted, focussedEl);
                let count = 0;
                while (nextElement !== focussedEl && (nextElement.disabled || nextElement.style.display === "none") && (count < this.optionElsSorted.length)) {
                    nextElement = webcomponentHelpers.getNextFromList(this.optionElsSorted, nextElement);
                    count++;
                }
        ...
        if (optionsByFirstLetter.length) {
            ...
            if (option.disabled || option === this.focussedEl) {
                for (let i = 0; i < optionsByFirstLetter.length; i++) {
                    option = webcomponentHelpers.getNextFromList(optionsByFirstLetter, optionsByFirstLetter[startIndex]);
                    if (option.disabled) {
                        option = null;
                    }
                    else {
     ...
     */
    isSelectEl(el) {
        return !!webcomponentHelpers.closest(el, this.el);
    }
    /**
    ...
    setInvokeSelectCallback() {
        this.invokeSelectCallback = webcomponentHelpers.parseFunction(this.selectCallback);
    }
    ...
    onHeaderClick(e) {
        ...
        const didClickOnSdxInputEl = !!webcomponentHelpers.closest(targetEl, this.filterInputEl);
    ...
    render() {
        return (index.h(index.Host, { "aria-expanded": (this.display === "open").toString() }, index.h("div", { class: this.getComponentClassNames(), ref: (el) => this.componentEl = el }, this.label && index.h("label", { class: "label", onClick: () => this.toggle() }, this.label, " ", this.required && index.h("span", { "aria-hidden": "true" }, "*")), index.h("div", { class: "wrapper", ref: (el) => this.wrapperEl = el }, index.h("div", { class: "header-wrapper" }, index.h("div", { class: "header", onClick: (e) => this.onHeaderClick(e) }, index.h("div", { class: "selection" }, this.isFilterable()
            ? (index.h("sdx-input", Object.assign({}, this.getDefaultInputFieldProps(), { value: this.filterInputElValue, ref: (el) => this.filterInputEl = el, changeCallback: (value) => this.onFilterInputElChange(value), inputCallback: (value) => this.onFilterInputElInput(value), focusCallback: () => this.onFilterInputElFocus(), blurCallback: () => this.onFilterInputElBlur(), autocomplete: "off", placeholder: this.placeholder, selectTextOnFocus: this.isKeyboardBehavior("filter"), inputStyle: this.getInputStyle() })))
            : (index.h("sdx-input", Object.assign({}, this.getDefaultInputFieldProps(), { value: this.getFormattedSelection() || this.placeholder, editable: false, inputStyle: Object.assign(Object.assign({}, this.getInputStyle()), { color: this.isOpenOrOpening() ? "#1781e3" : "" // $color-int-blue
                }) })))), (!this.isAutocomplete() || this.loading) &&
            index.h("div", { class: "thumb" }, this.loading
                ? index.h("sdx-loading-spinner", null)
                : index.h("div", { class: "icon" })))), index.h("div", { class: "list-container", ref: (el) => this.listContainerEl = el, tabIndex: -1 }, index.h("div", { class: "list" }, this.showPlaceholder() &&
            index.h("sdx-select-option", { placeholder: true }, this.placeholder), index.h("div", { class: "slot" }, index.h("slot", null)), this.isValidFilter(this.filter) && this.getMatchingOptionElsCount() === 0 &&
            index.h("div", { class: "no-matches-found" }, this.noMatchesFoundLabel)))), this.validationMessage &&
