...
const menuFlyoutReducer = (state = {}, action) => {
    switch (action.type) {
        ...
        case "TOGGLE_ARROW_EL":
            return Object.assign(Object.assign({}, state), { arrowEls: immutabilityHelpers.toggle(state.arrowEls, action.arrowEl) });
        default:
            return state;
    }
};
...
const MenuFlyout = class {
    ...
    componentWillLoad() {
        this.setInvokeDisplayChangeCallback();
        this.store = storeHelpers.createAndInstallStore(this, menuFlyoutReducer, this.getInitialState());
    ...
    open() {
        return new Promise((resolve) => {
            ...
            const animationOffset = this.directionState === "top-left" || this.directionState === "top-right"
                ? -this.offset
                : this.offset;
            anime_es.anime.remove(contentEl);
            anime_es.anime({
                targets: contentEl,
    ...
    setInvokeDisplayChangeCallback() {
        this.invokeDisplayChangeCallback = webcomponentHelpers.parseFunction(this.displayChangeCallback);
    }
...
exports.sdx_menu_flyout = MenuFlyout;
